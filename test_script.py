...
def safe_operation(operation_name, operation_func, delay=0.5):
    ...
    try:
        ...
        print(f"   â {operation_name} succeeded!")
        return result, True
    except Exception as e:
        print(f"   â {operation_name} failed: {e}")
...
def test_autocad_slow_steady():
    ...
    try:
        ...
        print("   â Connected to AutoCAD")
        time.sleep(1)

        # Step 2: Get document
        print("2. Getting active document...")
        doc = acad.ActiveDocument
        print(f"   â Document: {doc.Name}")
        time.sleep(1)
        ...
        print(f"   â ModelSpace has {initial_count} entities")
        time.sleep(1)
        ...
        print(f"   â Created {objects_created} new objects")

        # Zoom extents
        try:
            acad.ZoomExtents()
            print("   â Zoom extents completed")
        except Exception as e:
            print(f"   â Zoom extents failed: {e}")
        ...
        if len(successful_operations) > 0:
            print("ð SUCCESS! Some operations worked!")
            print("Working operations:")
            for op in successful_operations:
                print(f"   â {op}")
            return True
        else:
            print("â No operations succeeded.")
            return False
